...
namespace AppCenterEditor
{
    ...
    public class AppCenterEditorPrefsSO : ScriptableObject
    {
        ...
        {
            get => _latestEdExVersion;
            set
            {
                _latestEdExVersion = value;
                _lastEdExVersionCheck = DateTime.UtcNow;
                PlayerPrefs.SetString(EdExLastCheckDateKey,
                    _lastEdExVersionCheck.ToString(CultureInfo.InvariantCulture));
            }
        }

        public DateTime EdSet_lastSdkVersionCheck => PlayerPrefs.HasKey(SdkLastCheckDateKey)
            ? DateTime.Parse(PlayerPrefs.GetString(SdkLastCheckDateKey), CultureInfo.InvariantCulture)
            : _lastSdkVersionCheck;

        public DateTime EdSet_lastEdExVersionCheck => PlayerPrefs.HasKey(EdExLastCheckDateKey)
            ? DateTime.Parse(PlayerPrefs.GetString(EdExLastCheckDateKey), CultureInfo.InvariantCulture)
            : _lastEdExVersionCheck;
            ...
            get
            {
                ...
                if (!Directory.Exists(Path.Combine(Application.dataPath, "AppCenterEditorExtensions/Editor/Resources")))
                    Directory.CreateDirectory(Path.Combine(Application.dataPath,
                        "AppCenterEditorExtensions/Editor/Resources"));
                AssetDatabase.CreateAsset(_instance,
                    "Assets/AppCenterEditorExtensions/Editor/Resources/AppCenterEditorPrefsSO.asset");
