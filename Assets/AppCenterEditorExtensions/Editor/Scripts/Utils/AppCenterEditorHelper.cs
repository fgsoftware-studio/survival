...
namespace AppCenterEditor
{
    ...
    public static partial class AppCenterEditorHelper
    {
        ...
        private static GUISkin GetUiStyle()
        {
            ...
            {
                var loadPath = eachPath.Substring(eachPath.LastIndexOf("Assets" + Path.DirectorySeparatorChar,
        ...
        {
            var directoryList =
                Directory.GetDirectories(Application.dataPath, "*" + EDEX_NAME, SearchOption.AllDirectories);
            if (directoryList.Length == 0) throw new DirectoryNotFoundException(EDEX_NAME + " not found");

            var relativePath = directoryList[0].Substring(directoryList[0]
                .LastIndexOf("Assets" + Path.DirectorySeparatorChar, StringComparison.Ordinal));
