 ...
 DynamicObject,
        ...
        static class AppCenterSimpleJson
    {
        ...
        private static object ParseNumber(char[] json, ref int index, ref bool success)
        {
            ...
            {
                ulong number;
                success = ulong.TryParse(new string(json, index, charLength), NumberStyles.Any,
                    CultureInfo.InvariantCulture, out number);
                returnNumber = number;
            }
            else
        ...
        private static TokenType LookAhead(char[] json, int index)
        {
            var saveIndex = index;
        ...
        private static TokenType NextToken(char[] json, ref int index)
        {
            EatWhitespace(json, ref index);
            if (index == json.Length)
...
#if NETFX_CORE
                var isStringKeyDictionary =
 type.GetTypeInfo().IsGenericType && type.GetGenericTypeDefinition() == typeof(Dictionary<,>) && genArgs[0] == typeof(string);
#else
                var isStringKeyDictionary = type.IsGenericType &&
 ...
 new DataContractJsonSerializerStrategy());
        ...
        class PocoJsonSerializerStrategy : IJsonSerializerStrategy
    {
        ...
        public virtual object DeserializeObject(object value, Type type)
        {
            ...
            var isNumberType = AppCenterSimpleJson.NumberTypes.IndexOf(type) != -1;
#if NETFX_CORE
            bool isEnumType = type.GetTypeInfo().IsEnum;
#else
            var isEnumType = type.IsEnum; //type.GetType;
#endif
