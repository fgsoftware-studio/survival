...
namespace AppCenterEditor
{
    ...
    {
        public static void ImportLatestSDK(IEnumerable<AppCenterSDKPackage> packagesToImport, string version,
            string existingSdkPath = null)
        {
            try
            {
                var downloadUrls = new List<string>();
                foreach (var package in packagesToImport) downloadUrls.Add(package.GetDownloadUrl(version));
                AppCenterEditorHttp.MakeDownloadCall(downloadUrls, downloadedFiles =>
                {
                    try
                    {
                        ...
                        AppCenterEditorPrefsSO.Instance.SdkPath = string.IsNullOrEmpty(existingSdkPath)
                            ? AppCenterEditorHelper.DEFAULT_SDK_LOCATION
                            : existingSdkPath;
