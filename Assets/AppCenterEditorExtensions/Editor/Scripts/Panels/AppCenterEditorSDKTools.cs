...
namespace AppCenterEditor
{
    public class AppCenterEditorSDKTools : Editor
    {
        ...
        public static bool IsInstalled => AreSomePackagesInstalled();
        public static bool IsFullSDK => CheckIfAllPackagesInstalled();
        ...
        public static Object SdkFolder { get; private set; }
        public static string InstalledSdkVersion { get; private set; }
        public static GUIStyle TitleStyle => new GUIStyle(AppCenterEditorHelper.uiStyle.GetStyle("titleLabel"));

        private static Type appCenterSettingsType;
        private static bool isInitialized; // used to check once, gets reset after each compile
        private static Object _previousSdkFolderPath;
        ...
        public static SDKState GetSDKState()
        {
            ...
            //SDK installed.
            if (IsFullSDK) return SDKState.SDKIsFull;
        ...
        public static void DisplayPackagePanel(AppCenterSDKPackage sdkPackage)
        {
            using (new AppCenterGuiFieldHelper.UnityVertical(AppCenterEditorHelper.uiStyle.GetStyle("gpStyleGray1")))
            {
                using (new AppCenterGuiFieldHelper.UnityHorizontal(
                           AppCenterEditorHelper.uiStyle.GetStyle("gpStyleClear")))
                {
        ...
        public static void ShowUpgradePanel()
        {
            if (!sdkFolderNotFound)
                using (new AppCenterGuiFieldHelper.UnityVertical(
                           AppCenterEditorHelper.uiStyle.GetStyle("gpStyleGray1")))
                {
                    var versionNumber = !string.IsNullOrEmpty(InstalledSdkVersion)
                        ? InstalledSdkVersion.Split('.')
                        : new string[0];

                    var numerical = 0;
                    var isEmptyVersion = string.IsNullOrEmpty(InstalledSdkVersion) || versionNumber == null ||
                                         versionNumber.Length == 0;
                    if (isEmptyVersion || versionNumber.Length > 0 && int.TryParse(versionNumber[0], out numerical) &&
                        numerical < 0)
                        ...
                        {
                            GUILayout.FlexibleSpace();
                            EditorGUILayout.LabelField("SDK is outdated. Consider upgrading to the get most features.",
                    ...
                    if (ShowSDKUpgrade())
                    {
                        if (IsUpgrading)
                            ...
                            {
                                GUILayout.FlexibleSpace();
                                var image = DrawUtils.RotateImage(
                                    AssetDatabase.LoadAssetAtPath(
                                        "Assets/AppCenterEditorExtensions/Editor/UI/Images/wheel.png",
                                        typeof(Texture2D)) as Texture2D, angle++);
                                GUILayout.Button(new GUIContent("  Upgrading to " + LatestSdkVersion, image),
                                    AppCenterEditorHelper.uiStyle.GetStyle("Button"), GUILayout.MaxWidth(buttonWidth),
                            ...
                            {
                                GUILayout.FlexibleSpace();
                                if (GUILayout.Button("Upgrade to " + LatestSdkVersion,
                                        AppCenterEditorHelper.uiStyle.GetStyle("Button"), GUILayout.MinHeight(32)))
                                {
                                    IsUpgrading = true;
                                    UpgradeSdk();
                                }
                    ...
                    {
                        GUILayout.FlexibleSpace();
                        if (GUILayout.Button("VIEW RELEASE NOTES", AppCenterEditorHelper.uiStyle.GetStyle("textButton"),
                                GUILayout.MinHeight(32), GUILayout.MinWidth(200)))
            ...
            if (!sdkFolderNotFound)
                           ...
                           AppCenterEditorHelper.uiStyle.GetStyle("gpStyleClear")))
                {
                    ...
                    if (GUILayout.Button("REMOVE SDK", AppCenterEditorHelper.uiStyle.GetStyle("textButton"),
                            GUILayout.MinHeight(32), GUILayout.MinWidth(200))) RemoveSdk();
        ...
        private static void ShowFolderObject()
        {
            ...
            GUILayout.Space(5);
            using (new AppCenterGuiFieldHelper.UnityHorizontal(
                       AppCenterEditorHelper.uiStyle.GetStyle("gpStyleClearWithleftPad")))
            {
                GUILayout.FlexibleSpace();
                SdkFolder = EditorGUILayout.ObjectField(SdkFolder, typeof(Object), false, GUILayout.MaxWidth(200));
                GUILayout.FlexibleSpace();
            }
        ...
        private static void ShowSdkInstalledLabel()
        {
            GUILayout.Space(5);
            EditorGUILayout.LabelField(
                string.Format("SDK {0} is installed",
                    string.IsNullOrEmpty(InstalledSdkVersion) ? Constants.UnknownVersion : InstalledSdkVersion),
        ...
        private static void ShowInstallingButton()
        {
            ...
            using (new AppCenterGuiFieldHelper.UnityHorizontal(AppCenterEditorHelper.uiStyle.GetStyle("gpStyleEmpty")))
            {
                GUILayout.FlexibleSpace();
                var image = DrawUtils.RotateImage(
                    AssetDatabase.LoadAssetAtPath("Assets/AppCenterEditorExtensions/Editor/UI/Images/wheel.png",
                        typeof(Texture2D)) as Texture2D, angle++);
                GUILayout.Button(new GUIContent("  SDK is installing", image),
                    AppCenterEditorHelper.uiStyle.GetStyle("Button"), GUILayout.MaxWidth(buttonWidth),
                    GUILayout.MinHeight(32));
                GUILayout.FlexibleSpace();
            }
        ...
        private static void ShowInstallButton()
        {
            ...
            {
                GUILayout.FlexibleSpace();
                if (GUILayout.Button("Install all App Center SDK packages",
                        AppCenterEditorHelper.uiStyle.GetStyle("Button"), GUILayout.MaxWidth(buttonWidth),
                        GUILayout.MinHeight(32)))
                {
                    IsInstalling = true;
        ...
        {
            var notInstalledPackages = new List<AppCenterSDKPackage>();
            if (!IsInstalled)
            {
        ...
        {
            if (string.IsNullOrEmpty(LatestSdkVersion) || LatestSdkVersion == Constants.UnknownVersion) return false;
            ...
            foreach (var package in AppCenterSDKPackage.SupportedPackages)
                if (package.IsInstalled)
                {
                    var packageVersion = package.InstalledVersion;
                    var isPackageOutdated = false;
                    ...
                    {
                        var current = packageVersion.Split('.');
                        var latest = LatestSdkVersion.Split('.');
                    ...
                    if (isPackageOutdated) isOutdated = true;
                }
        ...
        {
            if (EditorUtility.DisplayDialog("Confirm SDK Upgrade",
                    "This action will remove the current App Center SDK and install the lastet version.", "Confirm",
                    "Cancel"))
                try
                {
                    ...
                    PackagesInstaller.ImportLatestSDK(installedPackages, LatestSdkVersion,
                        AppCenterEditorPrefsSO.Instance.SdkPath);
                }
                catch (Exception exception)
                {
        ...
        {
            if (prompt && !EditorUtility.DisplayDialog("Confirm SDK Removal",
                    "This action will remove the current App Center SDK.", "Confirm", "Cancel")) return;
            ...
            if (FileUtil.DeleteFileOrDirectory(AppCenterEditorPrefsSO.Instance.SdkPath))
            {
                ...
                if (prompt) AssetDatabase.Refresh();
            }
            else
            {
                AppCenterEditor.RaiseStateUpdate(AppCenterEditor.EdExStates.OnError,
                    "An unknown error occured and the App Center SDK could not be removed.");
            }
        }
        ...
        private static void RemoveAndroidSettings()
        {
            ...
            {
                var files = Directory.GetFiles(Application.dataPath + "/Plugins/Android/res/values",
                    "appcenter-settings.xml*", SearchOption.AllDirectories);
                foreach (var file in files) FileUtil.DeleteFileOrDirectory(file);
            }
        }
            ...
            foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
                ...
                catch (ReflectionTypeLoadException)
                {
                    ...
                    if (assembly.FullName
                        .StartsWith("Assembly-CSharp")) // The standard "source-code in unity proj" assembly name
                        EdExLogger.LoggerInstance.LogWarning(
                            "App Center Editor Extension error, failed to access the main CSharp assembly that probably contains App Center SDK");
                }
        ...
        {
            var threshold = AppCenterEditorPrefsSO.Instance.EdSet_lastSdkVersionCheck != DateTime.MinValue
                ? AppCenterEditorPrefsSO.Instance.EdSet_lastSdkVersionCheck.AddHours(1)
                : DateTime.MinValue;

            if (DateTime.Today > threshold)
                AppCenterEditorHttp.MakeGitHubApiCall(
                    "https://api.github.com/repos/Microsoft/AppCenter-SDK-Unity/git/refs/tags", version =>
                    {
        ...
        private static Object FindSdkAsset()
        {
            Object sdkAsset = null;

            // look in editor prefs
            if (AppCenterEditorPrefsSO.Instance.SdkPath != null)
                sdkAsset = AssetDatabase.LoadAssetAtPath(AppCenterEditorPrefsSO.Instance.SdkPath, typeof(Object));
            if (sdkAsset != null)
                return sdkAsset;

            sdkAsset = AssetDatabase.LoadAssetAtPath(AppCenterEditorHelper.DEFAULT_SDK_LOCATION, typeof(Object));
            if (sdkAsset != null)
                return sdkAsset;
            ...
            return AssetDatabase.LoadAssetAtPath(relPath, typeof(Object));
        }
    }
}