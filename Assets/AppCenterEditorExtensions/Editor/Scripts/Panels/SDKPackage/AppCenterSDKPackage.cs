    ...
    public abstract class AppCenterSDKPackage
    {
        ...
        private void RemovePackage(bool prompt = true)
        {
            if (prompt && !EditorUtility.DisplayDialog("Confirm SDK Removal",
                    string.Format("This action will remove the current {0} SDK.", Name), "Confirm", "Cancel")) return;
            EdExLogger.LoggerInstance.LogWithTimeStamp(string.Format("Removing {0} package...", Name));

            var toDelete = new List<string>();
            var pluginsPath = Path.Combine(AppCenterEditorPrefsSO.Instance.SdkPath, "Plugins");
            var androidPath = Path.Combine(pluginsPath, "Android");
            var sdkPath = Path.Combine(pluginsPath, "AppCenterSDK");
            var iosPath = Path.Combine(pluginsPath, "iOS");
            var wsaPath = Path.Combine(pluginsPath, "WSA");
            ...
            if (installedPackages.Count <= 1) AppCenterEditorSDKTools.RemoveSdk(false);

            if (deleted)
            {
                EdExLogger.LoggerInstance.LogWithTimeStamp(string.Format("{0} package removed.", Name));
                AppCenterEditor.RaiseStateUpdate(AppCenterEditor.EdExStates.OnSuccess,
                    string.Format("App Center {0} SDK removed.", Name));
            ...
            else
            {
                AppCenterEditor.RaiseStateUpdate(AppCenterEditor.EdExStates.OnError,
                    string.Format("An unknown error occured and the {0} SDK could not be removed.", Name));
            }
        }
        ...
        public void ShowPackageInstalledMenu()
        {
            ...
            using (new AppCenterGuiFieldHelper.UnityVertical(AppCenterEditorHelper.uiStyle.GetStyle("gpStyleEmpty")))
            {
                ...
                using (new AppCenterGuiFieldHelper.UnityHorizontal(
                           AppCenterEditorHelper.uiStyle.GetStyle("gpStyleEmpty")))
                {
                    GUILayout.FlexibleSpace();
                    var labelStyle = new GUIStyle(AppCenterEditorHelper.uiStyle.GetStyle("versionText"));
                    EditorGUILayout.LabelField(string.Format("{0} SDK {1} is installed", Name, sdkPackageVersion),
                        labelStyle);
                    GUILayout.FlexibleSpace();
                }

                var packageVersionIsValid = sdkPackageVersion != null && sdkPackageVersion != Constants.UnknownVersion;
                if (packageVersionIsValid &&
                    sdkPackageVersion.CompareTo(AppCenterEditorSDKTools.InstalledSdkVersion) != 0)
                    using (new AppCenterGuiFieldHelper.UnityHorizontal(
                               AppCenterEditorHelper.uiStyle.GetStyle("gpStyleEmpty")))
                    {
                        GUILayout.FlexibleSpace();
                        EditorGUILayout.LabelField(
                            "Warning! Package version is not equal to the AppCenter Core SDK version. ",
            ...
            using (new AppCenterGuiFieldHelper.UnityVertical(AppCenterEditorHelper.uiStyle.GetStyle("gpStyleEmpty")))
            {
                           ...
                           AppCenterEditorHelper.uiStyle.GetStyle("gpStyleEmpty")))
                {
                    ...
                    if (IsPackageInstalling)
                    {
                        ...
                        GUILayout.Button(new GUIContent(string.Format("  {0} SDK is installing", Name), image),
                            AppCenterEditorHelper.uiStyle.GetStyle("customButton"), GUILayout.MaxWidth(200),
        ...
        public void GetInstalledVersion(Type type, string coreVersion)
        {
            ...
            if (string.IsNullOrEmpty(InstalledVersion)) InstalledVersion = Constants.UnknownVersion;
        }
