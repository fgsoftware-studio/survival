...
namespace AppCenterEditor
{
    public class AppCenterEditor : EditorWindow
    {
        ...
        private const string EditorExtensionsDownloadFormat =
            "https://github.com/Microsoft/AppCenter-SDK-Unity-Extension/releases/download/{0}/AppCenterEditorExtensions-v{0}.unitypackage";
        ...
        public static float InnerContainerWidth => scrollInnerContainer.width;
        ...
        private static void AppCenterServices()
        {
        ...
        private static IEnumerator OpenAppCenterServices()
        {
            yield return new WaitForSeconds(1f);
            if (!Application.isPlaying) AppCenterServices();
        }
        ...
        private void OnGuiInternal()
        {
            ...
            scrollPosition = GUILayout.BeginScrollView(scrollPosition, false, false,
                GUILayout.Width(window.position.width), GUILayout.Height(window.position.height));
                       ...
                       GUILayout.Height(scrollInnerContainer.height)))
            {
                ...
                var state = AppCenterEditorSDKTools.GetSDKState();
                if (state == AppCenterEditorSDKTools.SDKState.SDKIsFull ||
                    state == AppCenterEditorSDKTools.SDKState.SDKNotFull) AppCenterEditorSDKTools.ShowUpgradePanel();
        ...
        {
            if (blockingRequests.Count > 0 || EditorApplication.isCompiling) return false;
            var state = AppCenterEditorSDKTools.GetSDKState();
            return
        ...
        private static void DisplayEditorExtensionHelpMenu()
        {
            using (new AppCenterGuiFieldHelper.UnityVertical(AppCenterEditorHelper.uiStyle.GetStyle("gpStyleGray1")))
            {
                           ...
                           AppCenterEditorHelper.uiStyle.GetStyle("gpStyleClear")))
                {
                    ...
                    EditorGUILayout.LabelField("App Center Editor Extensions: " + AppCenterEditorHelper.EDEX_VERSION,
                        AppCenterEditorHelper.uiStyle.GetStyle("versionText"));
                    GUILayout.FlexibleSpace();
                }
                ...
                if (ShowEdExUpgrade())
                    ...
                    {
                        GUILayout.FlexibleSpace();
                        if (GUILayout.Button("UPGRADE EDITOR EXTENSION",
                                AppCenterEditorHelper.uiStyle.GetStyle("textButtonMagenta"))) UpgradeEdEx();
                        GUILayout.FlexibleSpace();
                    }
                ...
                if (!string.IsNullOrEmpty(AppCenterEditorHelper.EDEX_ROOT))
                    ...
                    {
                        GUILayout.FlexibleSpace();
                        if (GUILayout.Button("UNINSTALL EDITOR EXTENSION",
                                AppCenterEditorHelper.uiStyle.GetStyle("textButton"))) RemoveEdEx();
                        GUILayout.FlexibleSpace();
                    }
            }
        }
        ...
        {
            if (EdExStateUpdate != null) EdExStateUpdate(state, status, json);
        }
        ...
        {
            var threshold = AppCenterEditorPrefsSO.Instance.EdSet_lastEdExVersionCheck != DateTime.MinValue
                ? AppCenterEditorPrefsSO.Instance.EdSet_lastEdExVersionCheck.AddHours(1)
                : DateTime.MinValue;

            if (DateTime.Today > threshold)
                AppCenterEditorHttp.MakeGitHubApiCall(
                    "https://api.github.com/repos/Microsoft/AppCenter-SDK-Unity-Extension/git/refs/tags", version =>
                    {
        ...
        private static bool ShowEdExUpgrade()
        {
            ...
            if (string.IsNullOrEmpty(AppCenterEditorHelper.EDEX_VERSION) ||
                AppCenterEditorHelper.EDEX_VERSION == Constants.UnknownVersion)
                return true;

            var currrent = AppCenterEditorHelper.EDEX_VERSION.Split('.');
        ...
        private static void RemoveEdEx(bool prompt = true)
        {
            if (prompt && !EditorUtility.DisplayDialog("Confirm Editor Extensions Removal",
                    "This action will remove App Center Editor Extensions from the current project.", "Confirm",
                    "Cancel"))
                return;
        ...
        private static void UpgradeEdEx()
        {
            if (EditorUtility.DisplayDialog("Confirm EdEx Upgrade",
                    "This action will remove the current App Center Editor Extensions and install the lastet version.",
                    "Confirm", "Cancel"))
            {
                RemoveEdEx(false);
                ImportLatestEdEx();
