        ...
        public static IntPtr ToNativeAttachments(ErrorAttachmentLog[] logs)
        {
            ...
            foreach (var errorAttachmentLog in logs)
            {
                if (errorAttachmentLog != null)
                {
                    ...
                    {
                        nativeLog =
 app_center_unity_crashes_create_error_attachment_log_text(errorAttachmentLog.Text, errorAttachmentLog.FileName);
                    }
                    else
                    {
                        nativeLog =
 app_center_unity_crashes_create_error_attachment_log_binary(errorAttachmentLog.Data, errorAttachmentLog.Data.Length, errorAttachmentLog.FileName, errorAttachmentLog.ContentType);
                    }
