...
{
    private static readonly AndroidJavaClass _errorAttachmentLogClass =
 new AndroidJavaClass("com.microsoft.appcenter.crashes.ingestion.models.ErrorAttachmentLog");
    ...
    internal static AndroidJavaObject ToJavaAttachments(ErrorAttachmentLog[] attachments)
    {
        ...
        foreach (var errorAttachmentLog in attachments)
        {
            if (errorAttachmentLog != null)
            {
                ...
                {
                    nativeLog =
 _errorAttachmentLogClass.CallStatic<AndroidJavaObject>("attachmentWithText", errorAttachmentLog.Text, errorAttachmentLog.FileName);
                }
                else
                {
                    nativeLog =
 _errorAttachmentLogClass.CallStatic<AndroidJavaObject>("attachmentWithBinary", errorAttachmentLog.Data, errorAttachmentLog.FileName, errorAttachmentLog.ContentType);
                }
