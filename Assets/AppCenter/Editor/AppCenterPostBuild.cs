...
public class AppCenterPostBuild : IPostprocessBuildWithReport
{
    ...
    public static void FixIl2CppLogging(string pathToBuiltProject)
    {
        ...
        if (!File.Exists(destDebuggerPath)) throw new FileNotFoundException("Debugger.cpp file not found");
        ...
        if (lastIncludeLineIndex == -1) throw new Exception("Unexpected content of Debugger.cpp");
        ...
        if (logMethodLineIndex == -1) throw new Exception("Unexpected content of Debugger.cpp");
        ...
        if (isLoggingMethodLineIndex == -1) throw new Exception("Unexpected content of Debugger.cpp");
    ...
    {
        while (currentLineIndex <= lines.Count && !lines[currentLineIndex].Contains("{")) currentLineIndex++;
        if (currentLineIndex >= lines.Count) throw new Exception("Unexpected content of Debugger.cpp");
    ...
    private static int GetLastLineInMethodBody(List<string> lines, int currentLineIndex)
    {
        ...
        var bracketsBalance = lines[lineIndex - 1].Count(item => item == '{');
        ...
        if (bracketsBalance != 0) throw new Exception("Unexpected content of Debugger.cpp");
        return lineIndex - 1;
    }
    ...
    {
        var binaries =
            AssetDatabase.FindAssets("*", new[] {AppCenterSettingsContext.AppCenterPath + "/Plugins/WSA/IL2CPP"});
    ...
    private static void AddInternetClientCapability(string pathToBuiltProject)
    {
        ...
        {
            Debug.LogWarning("Failed to add the `InternetClient` capability, file `" + AppManifestFileName +
                             "` is not found");
            return;
        }
        ...
        {
            Debug.LogWarning("Failed to add the `InternetClient` capability, multiple `" + AppManifestFileName +
                             "` files found");
            return;
        }
        ...
        var capabilitiesElements = xmlFile.Root.Elements()
            .Where(element => element.Name.LocalName == CapabilitiesElement).ToList();
        if (capabilitiesElements.Count > 1)
        {
            Debug.LogWarning(
                "Failed to add the `InternetClient` capability, multiple `Capabilities` elements found inside `" +
                appManifestFilePath + "` file");
            return;
        }

        if (capabilitiesElements.Count == 0)
        {
            xmlFile.Root.Add(new XElement(XName.Get(CapabilitiesElement, defaultNamespace),
                GetInternetClientCapabilityElement(defaultNamespace)));
        }
    ...
    private static void OnPostprocessInfo(PlistDocumentWrapper info, AppCenterSettings settings)
    {
        if (settings.UseDistribute && AppCenter.Distribute != null)
        {
            ...
            var iOSURLSchemesMethod =
                playerSettingsClass.GetMethod("GetURLSchemes", BindingFlags.Static | BindingFlags.NonPublic);
            ...
            if (iOSURLSchemesMethod != null)
            {
                ...
                schemes.AddRange(schemesFromSettings.ToList());
            }
            ...
            if (settings.UseDistribute && AppCenter.Distribute != null)
            {
                ...
                setStringMethod.Invoke(urlType,
                    new object[] {"CFBundleURLName", ApplicationIdHelper.GetApplicationId()});
                var urlSchemes = urlType.GetType().GetMethod("CreateArray")
                    .Invoke(urlType, new[] {"CFBundleURLSchemes"});
