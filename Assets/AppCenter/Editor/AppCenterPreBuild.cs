...
public class AppCenterPreBuild : IPreprocessBuildWithReport
{
    ...
    public void OnPreprocessBuild(BuildTarget target, string path)
    {
        ...
        if (target == BuildTarget.Android)
        {
            ...
            if (AppCenter.Crashes != null) LinkModule(AppCenterSettingsContext.SettingsInstance.UseCrashes, "crashes");
        }
    }
    ...
    private void AddStartupCode(IAppCenterSettingsMaker settingsMaker)
    {
        ...
        settingsMaker.IsAllowNetworkRequests(settings.AllowNetworkRequests);
        settingsMaker.EnableManualSessionTracker(settings.EnableManualSessionTracker);
        if (settings.CustomLogUrl.UseCustomUrl) settingsMaker.SetLogUrl(settings.CustomLogUrl.Url);
        ...
        if (settings.UseAnalytics && settingsMaker.IsAnalyticsAvailable()) settingsMaker.StartAnalyticsClass();
        if (settings.UseCrashes && settingsMaker.IsCrashesAvailable()) settingsMaker.StartCrashesClass();
        if (settings.UseDistribute && settingsMaker.IsDistributeAvailable())
        {
            if (settings.CustomApiUrl.UseCustomUrl) settingsMaker.SetApiUrl(settings.CustomApiUrl.Url);
            if (settings.CustomInstallUrl.UseCustomUrl) settingsMaker.SetInstallUrl(settings.CustomInstallUrl.Url);
            if (settings.EnableDistributeForDebuggableBuild)
                settingsMaker.SetShouldEnableDistributeForDebuggableBuild();
            if (!settings.AutomaticCheckForUpdate) settingsMaker.SetDistributeDisableAutomaticCheckForUpdate();
        ...
        {
            var startupType = settingsMaker.IsStartFromAppCenterBehavior(advancedSettings)
                ? StartupType.Skip
                : advancedSettings.GetStartupType();
    ...
    private static void LinkModule(bool isEnabled, string moduleName)
    {
        ...
        var aarFileAsset =
            AssetDatabase.FindAssets(aarName, new[] {AppCenterSettingsContext.AppCenterPath + "/Plugins/Android"});
